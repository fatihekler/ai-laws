...
import csv, hashlib, io, re, subprocess, sys, urllib.request
import pandas as pd
...
for t in tables:
    ...
    if any('country or area' in c for c in cols) and any('m49 code' in c for c in cols):
        selected=t; break
if selected is None:
    print('UN_TABLE_COLUMNS')
    for i,t in enumerate(tables): print(i, list(flat_cols(t).columns)[:20])
...
us_rows=[]
for r in reader:
    ab=r['STUSAB'].strip().upper(); name=r['STATE_NAME'].strip(); fips=r['STATE'].strip()
    jid='US-'+ab
    if jid in existing_by_id:
        ...
        jtype='SUBNATIONAL_US_TERRITORY_OR_STATISTICAL_EQUIVALENT' if ab in {'AS','GU','MP','PR','VI'} else 'SUBNATIONAL_US_STATE_OR_DISTRICT'
        out={
          'jurisdiction_id':jid,
          'name':name,
          ...
          'notes':f'U.S. Census state/statistical-equivalent code {fips} live list confirmed {DATE}. Enumeration does not establish AI-specific law or legal equivalence among states, district and territories.'
        }
    us_rows.append(out)
assert len(us_rows)>=56, len(us_rows)

# Preserve non-country/non-US-subnational rows from existing registry (meta/org/regional union, etc.).
preserve=[]
for r in existing:
    jid=r['jurisdiction_id']
    if jid in {x['jurisdiction_id'] for x in un_rows}: continue
    if jid.startswith('US-') and jid in {x['jurisdiction_id'] for x in us_rows}: continue
    if jid=='GLOBAL-ALL': continue  # replaced by explicit global coverage meta below
    preserve.append(r)
...
# Deterministic output order: meta, organizations/regional rows, UN rows by id, U.S. subnationals by id.
...
with VAL.open('w',encoding='utf-8',newline='') as f:
    cols=['metric','value','state_or_limit']
    w=csv.DictWriter(f,fieldnames=cols,lineterminator='\n'); w.writeheader()
    vals=[
      ...
      ('US_TERRITORY_ROWS',sum(r['jurisdiction_id'] in {'US-AS','US-GU','US-MP','US-PR','US-VI'} for r in us_rows),'ENUMERATED'),
